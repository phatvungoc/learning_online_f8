...
export class ApiService {
  ...
  OUTSTANDING_COURSES_API = 'https://localhost:5001/api/1.0/Course/get-outstanding-course';
  ...
  search(name: string, level: string, price: string) {
    return this._http.get<SearchResults>(
      this.SEARCH_COURSES_API + `nameCourse=${name}&levelCourse=${level}&priceCourse=${price}&page=1` 
      );
  }

  searchByInstructorName(instructorName: string) {
    return this._http.get<SearchResults>(this.SEARCH_COURSES_API + instructorName);
  }

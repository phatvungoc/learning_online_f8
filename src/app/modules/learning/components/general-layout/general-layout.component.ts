...
export class GeneralLayoutComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.section$.subscribe((courses : any) => {
      if(courses.data) {
        ...
        this.titleLessionScroll = courses?.data?.sections[0]?.lessions[0]?.title;
        this.videoCurrent.height = 532;
        this.videoCurrent.width = 920;
      }
      this.videoId = courses?.data?.sections[0]?.lessions[0]?.link.split('/')[4];
  ...
  ngAfterViewInit(): void {
      setInterval(() => {
        this.durationCurrent = this.videoCurrent.getCurrentTime();
        this.durationCurrentTime = this._lessionService.convertHMS(this.durationCurrent);
        this.durationTotal = this.videoCurrent.getDuration();
        this.durationTotalTime = this._lessionService.convertHMS(this.durationTotal);
        if((this.videoCurrent.getCurrentTime() / this.videoCurrent.getDuration()) * 100 > 90) {
          this._lessionService.getQuizzDetail(this.sectionFirstHashCode).subscribe(data => {
            this._lessionService.updateLession(data.data[this.currentLession]).subscribe((data : any) => {
              if(data.statusCode === 200 && this.lessionList.videoComplete === false) {
                this.lessionList.totalLessionComplete += 1;
                this.lessionList.lessionProgressCompleted = ((this.lessionList.totalLessionComplete / 
                  this.lessionList.totalLession) * 100).toFixed() + '%';
                this.lessionList.lessionProgressCompletedNow = ((this.lessionList.totalLessionComplete / 
                  this.lessionList.totalLession) * 100).toFixed();
